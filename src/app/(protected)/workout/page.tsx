    ...
    try {
      if (!isCurrentlyDone) {
        ...
        await supabase.from("workout_history_logs").insert({
          ...
          actual_reps_completed: exercises.find(e => e.id === exId)?.target_reps || 0,
                ...
                {expanded && (
                  <div className="px-4 pb-5 pt-2 border-t border-white/5 animate-in fade-in">
                    ...
                    <div className="w-full h-44 bg-black rounded-xl mb-6 overflow-hidden border border-white/10">
                      <img 
                        src={lib.video_url || "https://images.unsplash.com/photo-1534438327276-14e5300c3a48?q=80&w=640&auto=format&fit=crop"} 
                        alt={lib.name}
...
export default function WorkoutPage() { return <Suspense fallback={null}><WorkoutContent /></Suspense>; }