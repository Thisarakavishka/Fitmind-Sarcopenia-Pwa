...
export default function ProgramsPage() {
  ...
  // 🌟 Expand plan to see the schedule inside it
  async function handleExpandPlan(planId: string) {
    ...
    const { data } = await supabase
      ...
      .select(
        `*, session_exercises(target_sets, target_reps, exercise_library(name))`,
      )
      .eq("plan_id", planId)
      ...
      <div className="space-y-4 pt-2">
        {plans.length === 0 ? (
          <div className="py-20 text-center border border-dashed border-white/10 rounded-2xl text-white/20 font-bold uppercase text-[10px]">
            No plans found in your history.
          </div>
        ) : (
          plans.map((plan) => {
            ...
            return (
              <div key={plan.id} className="space-y-2">
                <Card
                  className={`p-5 transition-all ${isActive ? "bg-primary/5 border-primary/30" : "bg-[#0a0a0a] border-white/5 hover:border-white/20"}`}
                >
                  <div
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <p
                          className={`text-[9px] font-black uppercase tracking-widest ${isActive ? "text-primary" : "text-white/40"}`}
                        >
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <span
                          className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${isAI ? "bg-blue-500/10 text-blue-400" : "bg-white/10 text-white/60"}`}
                        >
                  ...
                  {/* ACTION BUTTON: Reactivate inactive plans */}
                  {!isActive && (
                          ...
                          <div className="space-y-1 pl-2">
                            {session.session_exercises.map(
                              (ex: any, idx: number) => (
                                <div
                                  key={idx}
                                  className="flex justify-between items-center text-[10px] font-medium text-white/60"
                                >
